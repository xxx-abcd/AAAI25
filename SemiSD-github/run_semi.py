...
# from model.hsd_0811 import HSD
# from model.semid import HSD
from graph_data.data_process0706 import GraphData
# from graph_data.re_create_graph_ui import GraphData
from graph_data.create_transition_graph import SequenceGraph
...
def main(model_name, dataset, sub_model):
    ...
    config_file_list = ['config/semi.yml']
    ...
    if config['is_semi'] and 'HSD' in model_name:
        g = SequenceGraph(config, test_data, data_name)
        ori_graph, in_graph, out_graph, item_set, item_set_len = g.graphs
        device = train_data.dataset.inter_feat.interaction['item_id'].device
        config['ITEM_SET'] = item_set
        config['ORI_GRAPH'] = ori_graph.to_dense()
        config['IN_GRAPH'] = in_graph.to_dense()
        config['OUT_GRAPH'] = out_graph.to_dense()
        config['ITEM_SET_LEN'] = item_set_len
    ...
    if config['is_semi']:
        ...
        filtered_pretrained_dict = {k: v for k, v in checkpoint.items() if k in model_dict and ('embedding' or 'sub' in k)}
        model_dict.update(filtered_pretrained_dict)
        model.load_state_dict(model_dict)

    'train SSDRec and use the following codes for SemiSD'
    # if config['is_semi']:
    #     if config['is_dna']:
    #         checkpoint_file = config['pre_train_model_dict']['SSDRec'][config['dataset']][config['sub_model']]
    #     else:
    ...
    #     checkpoint = torch.load(checkpoint_file)['state_dict']
    #     model_dict = model.state_dict()
    #     filtered_pretrained_dict = {k: v for k, v in checkpoint.items() if k in model_dict and ('embedding' in k)}
    #     model_dict.update(filtered_pretrained_dict)
    #     model.load_state_dict(model_dict)
    ...
    record_model_result(data_name, model_name, parameter_dict, rst_dic, note, config_file_list,
...
def record_model_result(dataset, model_name, parameter_dict, rst_dic, note, config, saved_model_file):
    ...
    if 'HSD' in model_name:
        model_name += is_semi + is_dna + '_' + parameter_dict['sub_model']

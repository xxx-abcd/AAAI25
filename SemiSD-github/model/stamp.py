...
class STAMP(SequentialRecommender):
    ...
    def forward(self, item_seq, item_seq_len):
        # item_seq: batch x len
        # item_seq_len: batch
        item_seq_emb = self.item_embedding(item_seq)  # batch x len x dim
        last_inputs = self.gather_indexes(item_seq_emb, item_seq_len - 1)  # batch x dim
        org_memory = item_seq_emb
        ms = torch.div(torch.sum(org_memory, dim=1), item_seq_len.unsqueeze(1).float()) # batch x dim
        alpha = self.count_alpha(org_memory, last_inputs, ms) # batch x len
        vec = torch.matmul(alpha.unsqueeze(1), org_memory)  # batch x 1 x dim
        ma = vec.squeeze(1) + ms  # batch x dim
        hs = self.tanh(self.mlp_a(ma))  # batch x dim
        ht = self.tanh(self.mlp_b(last_inputs))  # batch x dim
        seq_output = hs * ht  # batch x dim
        return seq_output

...
class HSD(SequentialRecommender):
    ...
    def generate_pos_seq(self, item_seq, item_seq_len, item_level_score, seq_level_score, mask):
        ...
        soft_noisy_flag = item_level_soft_flag * seq_level_soft_flag
    ...
    def forward(self, interaction, best_flag=False, train_flag=True):
        ...
        item_set = interaction[self.ITEM_SET]
        user = interaction[self.USER_ID]
        node_embs = torch.concat(self.user_embedding(user).unsqueeze(1),self.item_embedding(item_set))
        ori_graph, in_graph, out_graph = interaction[self.GRAPH]
        graph_mask = torch.ones(item_set.shape, dtype=torch.float, device=item_seq.device) * item_set.gt(0)
        item_set_len = interaction[self.ITEM_SET_LEN].unsqueeze(1)

        pseudo_label, additional_loss = self.semi(item_seq_emb, seq_mask, item_seq_len, node_embs,
                    ori_graph, in_graph, out_graph, graph_mask, item_set_len, seq_norm_percent=0.2,
        ...
        'semi-learning'
        semi_loss_value = semi_loss(soft_noisy_flag,pseudo_label,mask,item_seq_len) + additional_loss


        return element_wise_reconstruction_loss, pos_seq, pos_seq_emb, pos_seq_len, neg_seq, neg_seq_len, embedding_level_denoising_emb, clean_seq_percent, semi_loss_value,  loss.sum(
            -1).mean()
...
class SemiLearning(nn.Module):
    def __init__(self, config, dataset, max_seq_len, embedding_size, hidden_size):
        super(SemiLearning, self).__init__()
        self.embedding_size = embedding_size
        self.hidden_size = hidden_size
        self.graph_predictor = GraphEdgePredictor(hidden_size, hidden_size, 3, 3)
    ...
    def forward(self, ori_item_seq_embs, seq_mask, seq_len, node_embs,
                                       ori_graph, in_graph, out_graph, graph_mask, set_len, seq_norm_percent=0.2,
                                       graph_norm_percent=0.2, tau=1e-3):
        items, graph_loss = self._pseudo_item(node_embs, ori_graph, in_graph, out_graph, graph_mask, set_len, graph_norm_percent,
                                              tau)
        positions, seq_loss = self._pseudo_position(ori_item_seq_embs, seq_mask, seq_len,
                                                    seq_norm_percent, tau)

        labels = positions

        loss = seq_loss + graph_loss
        return labels, loss

    def _pseudo_position(self, ori_item_seq_embs, seq_mask, seq_len, norm_percent=0.2, tau=1e-3):
        seq_loss, seq_losses = self._seq_reconstruct(ori_item_seq_embs, seq_mask, seq_len)
        positions = self._seq_loss_filter(seq_len, seq_losses, norm_percent, tau)
        return positions, seq_loss

    def _pseudo_item(self, node_embs, ori_graph, in_graph, out_graph, graph_mask, set_len, norm_percent=0.2, tau=1e-3):

        graph_loss, item_losses = self._graph_reconstruct(node_embs, ori_graph, in_graph, out_graph, graph_mask)
        items = self._graph_loss_filter(set_len, item_losses, norm_percent, tau)
        return items, graph_loss
    ...
    def _seq_reconstruct(self, ori_seq, mask, seq_len):
        '''
        ...
        x = x * weight
        gen_seq = torch.fft.irfft(x, n=seq_len, dim=1, norm='ortho')
        loss_fct = nn.MSELoss(reduction='none')
        ...
        seq_loss4compare = loss_fct(gen_seq * mask, ori_seq * mask).sum(-1) / seq_len.squeeze() + (
                1 - mask) * 2e32
        return seq_loss, seq_loss4compare
    ...
    def _graph_reconstruct(self, node_embs, ori_graph, in_graph, out_graph, graph_mask):
        ...
        :return:
        '''
        node_emb = node_embs * graph_mask
        graph_weight = ori_graph.dense()
        edge_mask = graph_mask.unsqueeze(2) * graph_mask.unsqueeze(1)

        MSE = nn.MSELoss(reduction='none')
        edge_logits,_ = GraphEdgePredictor(node_emb, out_graph, in_graph, graph_mask)
        edge_losses = MSE(edge_logits * edge_mask, graph_weight * edge_mask)

        node_losses4compare = (edge_losses * edge_mask).sum(dim=1) + (1 - edge_mask) * 2e32

        edge_loss = edge_losses * edge_mask.sum(-1).sum(-1) / edge_mask.sum(-1).sum(-1)
        return edge_loss, node_losses4compare

    def _seq_loss_filter(self, seq_len, loss, norm_percent=0.2, tau=1e-3):
        # loss: B X L
        topk = torch.floor(seq_len * norm_percent)
    ...
    def _graph_loss_filter(self, set_len, loss, norm_percent=0.2, tau=1e-3):
        topk = torch.floor(set_len * norm_percent)
        item_label = self._multi_hot_encoding(loss, topk, tau)
        return item_label
...
def semi_loss(soft_noisy_flag, multi_hot_label, mask, seq_len):
    criterion = nn.BCEWithLogitsLoss(reduction='none')
    loss = criterion(soft_noisy_flag, multi_hot_label) * mask
